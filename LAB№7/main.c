...
void drawChessboard(int n, float posX, float posY, float PosZ)
{
    ...
    glEnableClientState(GL_VERTEX_ARRAY);
        ...
        for (int i = -n; i < n; i++)
        {
            for (int j = -n; j < n; j++)
            {
                    if ((i+j) % 2 == 0) glColor3f(0.9f, 0.9f, 0.0f);
                    else glColor3f(0.4f, 0.03f, 0.6f);
                glPushMatrix();
...
void DrawCube1()
{
    glPushMatrix();
        ...
        glTranslatef(-1, -1, 1);/*ïîëîæåíèå êóáà*/
        Draw();
        glRotatef(90, 1,0,0);
        Draw();
        glRotatef(90, 1,0,0);
...
void DrawPrism(const int n)
{
    ...
    float vertieces[3*vn]; // Ýòî æå è íîðìàëè.
